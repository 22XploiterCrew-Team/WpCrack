...
_banner='\x1b[2J\x1b[H\x1b[J'+_logo+'WordPress Brute Force Fast Login\nAuthor : RandsX@22XploiterCrew\nE-Mail : dev@22xploitercrew.my.id\n'
start=time.time()
...
def completed(password='',found=_C):
	A='=';global start;print(_banner);B=time.time()-start;print('{}Target completed successfully{}'.format(F.YELLOW,F.WHITE))
	if found is _A:print('{}Successfully entered into the target dashboard with duration time is "{} seconds"{}'.format(F.GREEN,int(B),F.WHITE))
